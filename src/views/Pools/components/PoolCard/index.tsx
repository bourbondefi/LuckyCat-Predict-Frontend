...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  const { rate, title, sousId, stakingToken, earningToken, isFinished, userData } = pool
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          title={title}
          subTitle={
            isCakePool ? t('Earn CAKE, stake CAKE') : t('Buy Miners: %symbol%', { symbol: stakingToken.symbol })
          }
        />
